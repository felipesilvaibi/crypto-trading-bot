...
# importar candles
timeframe = "1m"
bars = helper.binance.fetch_ohlcv(symbol, timeframe, limit=35)
df_candles = pd.DataFrame(
    bars, columns=["time", "abertura", "max", "min", "fechamento", "volume"]
)
df_candles["time"] = pd.to_datetime(df_candles["time"], unit="ms", utc=True).map(
...
df_candles["VWAP"] = df_candles["price_weighted"].sum() / df_candles["volume"].sum()
...
if (
    ...
    and df_candles["MACD_12_26_9"].iloc[-1] >= df_candles["MACDs_12_26_9"].iloc[-1]
):
    if (
